        ...
        // end of Normal computing code
          ...
          */
          ...
          for (float s=0; s<=1.1; s=s+0.1 )
          {
              ...
              // Draw ORIGINAL Spine Curves
              current = Bez(B, A, C, s);  
              pt Origin=new pt(B.x,B.y,B.z);
              pt Eye=new pt(0,0,800);
              stroke(orange); 
              showEdge(previous,current);
              previous = current; 
              camera(previous.x, previous.y, previous.z, width/6, height/6, 0, 1, 1, 1); // This zooms into a bezier point, just need to figure out loop animation to cycle through all points
              endCamera();
          }
